      ...
      onSubmit: async (values) => {
        try {
          ...
          if (error.response) {
            ...
            setError(
              "An issue occurred while logging into your account. Please try again.",
            );
          }
          setUserLoginMessage("");
          setLoading(false);
    ...
    <div className="min-h-[100vh] w-full px-10 py-20 flex items-center justify-center">
      <div className="flex items-center justify-center w-full md:w-[70%] min-h-[70vh] mx-auto shadow-2xl rounded overflow-hidden">
        <div className="w-full h-full flex flex-col items-center justify-between px-2">
          {userLoginMessage && (
            <p className="text-successColorCode text-sm md:text-xl text-center">
              {userLoginMessage}
            </p>
          )}
          {error && (
            <p className="text-primaryErrorMessage text-sm md:text-xl text-center">
              {error}
            </p>
          )}
          {loading && (
          ...
          <h1 className="text-primaryBlackColor font-bold text-xl md:text-2xl capitalize">
            Sign In
          </h1>
          <form onSubmit={handleSubmit} className="w-full">
            ...
            <div className="w-full flex flex-col gap-4 items-center justify-center">
              ...
              <p className="md:hidden">
                ...
                <Link to="/sign-up" className="text-md font-semibold underline">
                  SIGN UP
                </Link>
              </p>
            </div>
        ...
        <div className="w-10/12 md:flex min-h-[70vh] bg-primaryBlueColor hidden flex-col items-center justify-center p-2 text-center text-primaryWhiteColor gap-3 rounded-tr-2xl rounded-br-2xl">
          <h1 className="text-2xl text-primaryWhiteColor md:text-4xl font-bold">
            Hello, Friend 👋
          </h1>
          <p className="text-tertiaryBlueColor text-sm md:text-md">
            We are Schools Mine, your trusted schools resource. Explore with us.{" "}
            <span className="font-semibold text-md">Let's Get Started</span>
          </p>
          <Link to="/sign-up">
            <Button title="Sign Up" border="border" />
          </Link>
        </div>
      </div>
    </div>
